...
const periodOrder: Array<keyof typeof periodLabels> = ['year', 'month', 'week', 'day']
    ...
    <AppShell>
      <main className="flow-page page-width">
        ...
        <article className="life-card">
          <div className="life-card__intro"><h2>Моя жизнь</h2><p>Фактическая продолжительность от даты рождения до сегодняшнего дня.</p><Link to="/" className="button button--ghost">Показать на карте</Link></div>
        ...
        <div className="flow-grid">
          {periodOrder.map((kind, index) => {
            const period = elapsedPeriod(kind, now)
            const bounds = currentPeriodBounds(kind, now)
            return <article className="flow-card" key={kind}><header><div><span className="flow-card__index">{String(index + 1).padStart(2, '0')}</span><h2>{periodLabels[kind]}</h2></div><strong>{period.percent.toFixed(2).replace('.', ',')}%</strong></header><div className="progress-track" aria-label={`${periodLabels[kind]} прошло на ${period.percent.toFixed(2)} процента`}><i style={{ width: `${period.percent}%` }} /></div><div className="flow-card__stats"><div><span>прошло</span><strong>{formatClockDuration(period.elapsedMs)}</strong></div><div><span>осталось</span><strong>{formatClockDuration(period.remainingMs)}</strong></div></div><details><summary>{periodNotes[kind]}</summary><p>{new Intl.DateTimeFormat('ru-RU', { dateStyle: 'medium', timeStyle: 'short' }).format(bounds.start)} — {new Intl.DateTimeFormat('ru-RU', { dateStyle: 'medium', timeStyle: 'short' }).format(bounds.end)}</p></details></article>
          })}
        </div>
      </main>
    </AppShell>
